package com.asqnw.autodial.util;
...
//            connection.setReadTimeout(60000);//设置读取远程返回的数据时间：60000毫秒
    ...
    public String postReqStr(String url, String body) throws HttpException.UnAuthorize, HttpException.Forbidden, HttpException.Unknown, HttpException.ServerError, UnknownHostException
    {
        return new String(postReqBytes(url, body), StandardCharsets.UTF_8);
    }

    public byte[] postReqBytes(String url, String body) throws HttpException.UnAuthorize, HttpException.Forbidden, HttpException.Unknown, HttpException.ServerError, UnknownHostException
    {
        try
        {
            final HttpURLConnection connection = (HttpURLConnection) new URL(url).openConnection();
...
//            connection.setReadTimeout(60000);//设置读取远程返回的数据时间：60000毫秒
        ...
        catch (HttpException.UnAuthorize | HttpException.Forbidden | HttpException.Unknown | HttpException.ServerError | UnknownHostException httpException)
        {
            throw httpException;
        }
        catch (Exception ignored)
